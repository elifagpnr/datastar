...
- Canlı text-only tahminler için yardımcı katman: pseudo-label ile eğitilmiş hash n-gram NLP text modeli.
...
- Destekleyici görseller: `risk_score_histogram.png`, `platform_normalized_risk.png`, `top_risk_authors.png`, `risk_funnel.png`, `temporal_burst_windows.png`.
...
- Jüriye anlatılacak hazır hikayeler için `artifacts/case_studies.md` kullanılmalı.
- Vurgu: aynı narrative signature, çoklu author, kısa zaman penceresi ve cluster confidence score.
- Not: çok dilli yayılım `cluster_language_nunique` ile görünür kılınır; çeviri semantiği iddia edilmez.

---

## 8. Canlı Demo
- Fonksiyon: `predict_live(text, language=None, url=None, author_hash=None, date=None, english_keywords=None)`.
- Çıktı: label, risk score, organic score, nlp_text_risk, top reasons, used features, psychological triggers.
...
- `temporal_burst_windows.png`: kampanya dönemlerini z-score sıçramalarıyla görünür kılar.
- `case_studies.md`: somut jüri anlatımı için üç gerçek örnek hikaye sunar.
...
- `temporal_burst_windows.png`: Saatlik risk z-score sıçramalarını ve Campaign Burst pencerelerini gösterir.
